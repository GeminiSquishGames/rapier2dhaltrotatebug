...
@export var torque_mag : float = 80000 * mass
...
# mouse turn
    ...
    if Input.is_action_pressed("pew") and $FireSpeed.time_left <= 0.0:
        ...
        pew_instance.apply_impulse(Vector2.from_angle(global_rotation) * pew_impulse_mag * pew_mag_mod * delta)
...
func mouse_rotation_control():
    ...
    apply_torque_impulse(-torque_mag * rot_factor * get_physics_process_delta_time())
